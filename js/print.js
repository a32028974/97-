// /js/print.js — v2025-09-06b (ticket + talón lateral estable)
(function () {
  // ===== Tamaños base =====
  const PAGE_W_MM = 130;
  const PAGE_H_MM = 155;
  ...
  // ===== Columnas: ticket izquierda + talón derecha =====
  const LEFT_W_MM  = 90;   // ← ticket un poco más angosto
  const GAP_MM     = 3;
  const RIGHT_W_MM = PAGE_W_MM - LEFT_W_MM - GAP_MM;

  // ===== Nudge por impresoras/UA =====
  const UA = navigator.userAgent || '';
  const IS_MOBILE = /Android|iPhone|iPad|iPod/i.test(UA);
  const NUDGE_TOP_MM  = IS_MOBILE ? -12 : -2; // ajustado suave en desktop
  ...
  function printInIframe(htmlInner, numero){
    ...
    <style>
      ...
      *{ -webkit-print-color-adjust: exact; print-color-adjust: exact; }
      ...
      /* Hoja → grid 2 columnas */
      .sheet{
        width:${PAGE_W_MM + EXTRA_W_MM}mm; height:${PAGE_H_MM + EXTRA_H_MM}mm;
        position:fixed; top:0; left:0; overflow:hidden;
        display:grid; grid-template-columns:${LEFT_W_MM}mm ${GAP_MM}mm ${RIGHT_W_MM}mm; grid-template-rows:100%;
      ...
      .sub{ color:var(--muted); font-size:8.5pt; margin-top:.2mm; }
      .nro{ justify-self:end; } .nro .lbl{ font-size:8pt; color:var(--muted); } .nro .val{ font-size:12pt; font-weight:700; }
      ...
      .kv .k{ color:var(--muted); font-size:8.5pt; } .kv .v{ font-weight:600; min-height:10pt; }
      .dtl{ margin-top:1mm; }
      ...
      /* Talón (flex column: datos → sumario → QR al fondo) */
      .slip{
        width:${RIGHT_W_MM}mm; height:${PAGE_H_MM}mm; box-sizing:border-box;
        border-left:.35mm dashed #9aa4b2;
      ...
      .slip-title{ font-weight:800; font-size:10pt; color:var(--brand); }
      .slip-sub{ font-size:8.5pt; color:var(--muted); line-height:1.2; }

      .slip-row{ display:grid; grid-template-columns:21mm 1fr; column-gap:2mm; }
      .slip-row .k{ color:var(--muted); font-size:8.5pt; }
